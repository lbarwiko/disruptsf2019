...
def transactionData(upto=23):
    ...
    return json.dumps({'spending':test.mean()[:upto].values.tolist(),
            'income':income,
            'upperRiskBand': (test.mean()+test.std())[:upto].values.tolist(),
            'lowerRiskBand': (test.mean()-test.std())[:upto].values.tolist(),
...
def decision(cost=500, upto=23):
    ...
    if (remain > 0) and (risk < 0):
        decision = {'decision': "Be cautious", 'reason':"You will likely over spend this month by -$" + str(round(abs(risk),2)) + "."}
    elif (remain > 0) and (risk > 0):
        decision = {'decision':"Yes", 'reason':"You will likely still have $"+ str(round(remain,2)) +" left by the end of the month."}
    elif (remain < 0):
        decision = {'decision':"No", 'reason':"You will likely add -$"+str(round(abs(remain),2))+" amount of debt at the end of the month."}
    ...
    return json.dumps({'spending':test.mean()[:upto].values.tolist(),
            ...
            'projectedUpperRiskBand': (test.mean()+test.std()+ cost)[:upto].values.tolist(),
            'projectedLowerRiskBand': (test.mean()-test.std()+ cost)[:upto].values.tolist(),
            'cateogories':{'Clothing': 7, 'Fast Food': 23, 'Restaurants':10, 'Parking':8, 
                           'Groceries':22, 'Shopping':18, 'Pets':12}})